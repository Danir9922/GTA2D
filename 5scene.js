...
var M5={phase:'idle',objectives:[],cur:0,reward:3000,called:false,bossTimer:0,bossLost:0,failing:false};
...
var BOSS_TIMEOUT=10800, BOSS_LOST_DIST=3000, BOSS_LOST_FRAMES=90;
var D5_CALL=[
{name:'🧔 Виктор',text:'Рэй, ты ещё жив? Слушай внимательно — это финал. Долли повёл товар сам, лично.',bg:'city',chars:[{x:.3,color:'#833',skin:'#dba',hair:'#222',label:'Виктор',scale:1.2}]},
{name:'🧔 Виктор',text:'Не по телефону такое. Приезжай ко мне, обсудим план лицом к лицу. Жду.',bg:'city',chars:[{x:.3,color:'#833',skin:'#dba',hair:'#222',label:'Виктор',scale:1.2}]},
{name:'😎 Рэй',text:'Понял. Еду к тебе, никуда не денусь.',bg:'city',chars:[{x:.3,color:'#833',skin:'#dba',hair:'#222',label:'Виктор',scale:1.2},{x:.7,color:'#25c',skin:'#fca',hair:'#432',label:'Рэй',scale:1.2}]},
{name:'',text:'▶ Доберись до оранжевой «5» у Виктора.',bg:'dark',chars:[]}];
var D5_START=[
{name:'🧔 Виктор',text:'Рэй... это конец. Долли лично повёл товар через город. Если упустим — нам крышка обоим.',bg:'city',chars:[{x:.3,color:'#833',skin:'#dba',hair:'#222',label:'Виктор',scale:1.2}]},
...
{name:'🧔 Виктор',text:'Сначала склад — зачисти охрану. Потом Долли поедет на тачке. Догони и сожги её. У тебя 3 минуты, не больше.',bg:'city',chars:[{x:.3,color:'#833',skin:'#dba',hair:'#222',label:'Виктор',scale:1.2}]},
{name:'🧔 Виктор',text:'После — его логово, там ещё охрана. Зачистишь — возвращайся ко мне. Три тысячи и свобода от банды навсегда.',bg:'city',chars:[{x:.3,color:'#833',skin:'#dba',hair:'#222',label:'Виктор',scale:1.2}]},
...
{name:'',text:'▶ ЦЕЛЬ: склад → погоня за боссом (3 мин) → логово → к Виктору.',bg:'dark',chars:[]}];
var D5_DONE=[
...
{name:'🧔 Виктор',text:'Полиция сняла перекрытие. Вся карта твоя — остров, река, аэропорт, всё открыто. Три тысячи, брат.',bg:'city',chars:[{x:.3,color:'#833',skin:'#dba',hair:'#222',label:'Виктор',scale:1.2}]},
...
function spawnBoss(){m5boss={x:WARE5.x,y:WARE5.y-2*BLOCK*TILE,angle:Math.PI/2,speed:3.4,hp:260,maxHp:260,turnT:rand(70,140)};}
...
function failM5(msg){m5boss=null;m5Ware.length=0;m5Lair.length=0;M5.failing=true;showMessage('❌ '+msg+' Миссия 5 заново.',3000);playSFX('wasted');setTimeout(function(){resetM5();},1700);}
function resetM5(){m5Ware.length=0;m5Lair.length=0;m5boss=null;M5.failing=false;spawnGroup(m5Ware,WARE5.x,WARE5.y,6);M5.phase='active';M5.bossTimer=BOSS_TIMEOUT;M5.bossLost=0;M5.objectives=[{text:'Доедь до склада Долли',done:false},{text:'Зачисти склад (0/6)',done:false},{text:'Догони и уничтожь машину босса (⏱3:00)',done:false},{text:'Зачисти логово (0/4)',done:false},{text:'Вернись к Виктору',done:false}];M5.cur=0;window.missionMarker.x=WARE5.x;window.missionMarker.y=WARE5.y;window.missionMarker.active=true;}
function updateMission5(){if(window.cutsceneSystem&&window.cutsceneSystem.active)return;
 if(M5.phase==='idle'&&window.M4&&window.M4.phase==='free'){if(!M5.called){M5.called=true;window.cutsceneSystem.start(D5_CALL,function(){M5.phase='trigger';window.missionMarker.x=TRIGGER5.x;window.missionMarker.y=TRIGGER5.y;window.missionMarker.active=true;showMission('📞 Доберись до оранжевой «5»!',3500);});}return;}
 ...
 if(M5.phase==='active'){
  ...
  else if(M5.cur===1){guardAI(m5Ware);var dw=m5Ware.filter(function(e){return !e.alive;}).length;M5.objectives[1].text='Зачисти склад ('+dw+'/'+m5Ware.length+')';if(dw>=m5Ware.length){M5.objectives[1].done=true;M5.cur=2;M5.bossTimer=BOSS_TIMEOUT;spawnBoss();showMission('🚗 Босс поехал! В тачку и догони! (3 мин)',3000);playSFX('select');}}
  else if(M5.cur===2){
   M5.bossTimer--;
   if(m5boss){var BS=BLOCK*TILE,RC=ROAD_W*TILE/2;var nrc=function(v){return Math.round((v-RC)/BS)*BS+RC;};var horiz=Math.abs(Math.cos(m5boss.angle))>.5;if(horiz)m5boss.y+=(nrc(m5boss.y)-m5boss.y)*.15;else m5boss.x+=(nrc(m5boss.x)-m5boss.x)*.15;var cx=nrc(m5boss.x),cy=nrc(m5boss.y),cross=Math.abs(m5boss.x-cx)<16&&Math.abs(m5boss.y-cy)<16;if(cross){m5boss.turnT--;if(m5boss.turnT<=0){m5boss.turnT=rand(70,140);if(Math.random()<.5){var dirs=[0,Math.PI/2,Math.PI,-Math.PI/2];var opts=dirs.filter(function(a){return Math.abs(Math.cos(a-m5boss.angle))<.5;});m5boss.angle=opts[randInt(0,opts.length-1)];}}}
    var bnx=m5boss.x+Math.cos(m5boss.angle)*m5boss.speed,bny=m5boss.y+Math.sin(m5boss.angle)*m5boss.speed;
    if(typeof isInWater==='function'&&isInWater(bnx,bny)){m5boss.angle+=Math.PI;}else if(typeof collidesBuilding==='function'&&collidesBuilding(bnx,bny,24)){m5boss.angle+=Math.PI;}else if(bnx<RC||bnx>WORLD_W-RC||bny<RC||bny>WORLD_H-RC){m5boss.angle+=Math.PI;}else {m5boss.x=bnx;m5boss.y=bny;}
    ...
    if(dist(player,m5boss)>BOSS_LOST_DIST){M5.bossLost++;if(M5.bossLost>BOSS_LOST_FRAMES){failM5('Потерял босса!');return;}}else M5.bossLost=0;
    if(M5.bossTimer<=0){failM5('Время вышло — босс ушёл!');return;}
...
function renderMission5World(ctx,W,H){if(window.cutsceneSystem&&window.cutsceneSystem.active)return;
 ...
 if(M5.phase==='active'){ctx.save();ctx.translate(-camera.x,-camera.y);drawGuards(ctx,m5Ware);drawGuards(ctx,m5Lair);
  ...
  ctx.restore();
  if(m5boss&&M5.cur===2){arrow5(ctx,W,H,m5boss.x,m5boss.y);var sec=Math.max(0,Math.ceil(M5.bossTimer/60));var mm=Math.floor(sec/60),ss=sec%60;ctx.fillStyle='rgba(0,0,0,.7)';ctx.fillRect(W/2-70,70,140,34);ctx.strokeStyle=sec<30?'#f44':'#fc0';ctx.lineWidth=2;ctx.strokeRect(W/2-70,70,140,34);ctx.fillStyle=sec<30?'#f44':'#fc0';ctx.font='bold 22px monospace';ctx.textAlign='center';ctx.fillText('⏱ '+mm+':'+(ss<10?'0':'')+ss,W/2,94);}
 }
...
window.missionRestarters.push(function(){if(M5.phase==='active'&&!M5.failing){resetM5();showMission('🔁 Миссия 5 начата заново',2000);}});
window.M5=M5;
console.log('5scene v2 ЗАГРУЖЕН');
})();
// ====== 5scene.js КОНЕЦ ======
