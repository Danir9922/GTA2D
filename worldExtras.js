...
window.onWorldReady=function(){
 ...
 console.log('worldExtras v9 ЗАГРУЖЕН');
};
...
function hijackCheck(){
 if(player.inVehicle||window.interiorOpen||(window.cutsceneSystem&&window.cutsceneSystem.active)||!player.alive)return;
 var best=null,bd=55;for(var i=0;i<trafficVehicles.length;i++){var v=trafficVehicles[i];if(v.driver){var d=dist(player,v);if(d<bd){bd=d;best=v;}}}
 if(!best)return;
 ...
 var idx=trafficVehicles.indexOf(best);if(idx>=0)trafficVehicles.splice(idx,1);
...
window.renderGroundExtras=function(ctx){
 ...
 if(typeof camera!=='undefined'){var cx=camera.x,cy=camera.y,W=canvas.width,H=canvas.height;ctx.fillStyle='#14507e';
  ...
  ctx.fillStyle='rgba(120,190,255,.12)';for(var i=0;i<14;i++){var wx=cx+((gameTime*0.3+i*137)%(W+200))-100;ctx.fillRect(wx,cy+ (i%2? 30: H-40),60,3);}}
...
window.renderMissionHUD=function(ctx,W,H){
 ...
 ctx.fillStyle='#0f0';ctx.font='10px monospace';ctx.textAlign='left';ctx.fillText('worldExtras v9',8,H-6);
...
window.drawMapLabels=function(ctx,SX,SY){
 ctx.fillStyle='#14507e';ctx.fillRect(SX(-4000),SY(-4000),WORLD_W+8000,4000);ctx.fillRect(SX(-4000),SY(WORLD_H),WORLD_W+8000,4000);ctx.fillRect(SX(-4000),SY(0),4000,WORLD_H);ctx.fillRect(SX(WORLD_W),SY(0),4000,WORLD_H);
 ...
 ctx.textAlign='center';ctx.fillStyle='#cfc';ctx.font='italic 20px Arial';ctx.fillText('🏝️ ДИКИЙ ОСТРОВ',SX(WORLD_W*0.5),SY(1*BS));ctx.fillStyle='#9cf';ctx.font='italic 22px Arial';ctx.fillText('р. Лос-Рио',SX(WORLD_W*0.5),SY(RIVER*BS+BS/2));
 if(!window.bridgesOpen){ctx.fillStyle='#f44';ctx.font='bold 14px Arial';ctx.fillText('🚧 МОСТЫ ЗАКРЫТЫ',SX(8*BS+RC/2),SY(PROLIV*BS+BS/2));ctx.fillText('🚧 МОСТЫ ЗАКРЫТЫ',SX(8*BS+RC/2),SY(RIVER*BS+BS/2));}
};
...
addEventListener('keydown',function(e){
 ...
 if(window.interiorOpen===null&&e.code==='KeyF'){hijackCheck();return;}
