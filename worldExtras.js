...
var EDGE_TOP=0, EDGE_BOT=15;
var bridgeN={x:8*BS, y:-32, w:RC, h:BS+64};
var bridgeS={x:8*BS, y:EDGE_BOT*BS-32, w:RC, h:BS+64};
...
function relocateFromWater(o){
 ...
 if(o.y<BS) o.y=1*BS+RC; else o.y=14*BS+RC;
 o.x=clamp(o.x,RC,WORLD_W-RC); o.angle=0; if(o.speed===undefined)o.speed=0;
}
window.onWorldReady=function(){
 for(var bx=0;bx<MAP_BLOCKS;bx++){window.clearBlock(bx,EDGE_TOP);window.clearBlock(bx,EDGE_BOT);}
 ...
 waterBodies.push({x:0,y:0,w:WORLD_W,h:BS,_ocean:true});
 waterBodies.push({x:0,y:EDGE_BOT*BS,w:WORLD_W,h:BS,_ocean:true});
...
window.drawMiniLabels=function(mc,s){mc.textAlign='center';mc.fillStyle='#7bf';mc.font='italic 9px Arial';mc.fillText('🌊 ОКЕАН',WORLD_W*0.5*s,(0.5*BS)*s);mc.fillText('🌊 ОКЕАН',WORLD_W*0.5*s,(15.5*BS)*s);};
window.drawMapLabels=function(ctx,SX,SY){ctx.textAlign='center';ctx.fillStyle='#7bf';ctx.font='bold 24px Arial';ctx.fillText('🌊 ОКЕАН',SX(WORLD_W*0.5),SY(0.5*BS));ctx.fillText('🌊 ОКЕАН',SX(WORLD_W*0.5),SY(15.5*BS));};
