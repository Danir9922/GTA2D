(function(){
const animals=[];
const ISLAND_Y1=3*BLOCK*TILE; // северный район = блоки 0-2
function inIsland(x,y){return y>=20&&y<=ISLAND_Y1-20&&x>=20&&x<=WORLD_W-20;}
function freeSpot(){ // точка в северном районе, не в здании
 for(let k=0;k<15;k++){const x=rand(40,WORLD_W-40),y=rand(40,ISLAND_Y1-40);if(!collidesBuilding(x,y,10)&&!isInWater(x,y))return{x,y};}
 return{x:8*BLOCK*TILE+TILE*2,y:1*BLOCK*TILE+TILE*2};
}
function spawnAnimals(){
 animals.length=0;
 const types=[['wolf','#777',4],['rabbit','#dcd',6],['bug','#0a0',8],['bug','#fa0',6]];
 types.forEach(t=>{for(let i=0;i<t[2];i++){const s=freeSpot();animals.push({kind:t[0],color:t[1],x:s.x,y:s.y,angle:rand(0,6.28),t:rand(40,160),hop:0});}});
}
